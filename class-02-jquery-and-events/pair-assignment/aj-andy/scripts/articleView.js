...
articleView.handleMainNav = function() {
  ...
  $('.main-nav').on('click', function(e){
    ...
    if(target.dataset.content == $('section').attr('id'));
    for(var i=0; i < $('li').length; i++){
      if(target.dataset.content == $('li')[i].dataset.content){
        if($('li')[i].dataset.content == 'articles') {
          $('#articles').show();
        } else if ($('li')[i].dataset.content == 'about') {
          $('#about').show();
        }
        console.log($('li')[i].dataset.content);
      }
    }
  });
